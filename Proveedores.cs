...
namespace Verduleria
{
    public partial class Proveedores : Form
    {
        ...
        private void grilla(String cadena)
        {
            ...
            con.ConnectionString = @"Data Source=localhost\SQL2022; Initial Catalog=verduleria; User ID = sa; Password = lab02; TrustServerCertificate = True";
            con.Open();
            dataprov = new SqlDataAdapter();
            dsprov = new DataSet();
            sqlprov = new SqlCommand(cadena, con);
            dataprov.SelectCommand = sqlprov;
            dataprov.TableMappings.Add("Table", "Proveedor");
            dataprov.Fill(dsprov);
            dgvproveedor.DataSource = dsprov;
        ...
        private void txtbusqueda_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                cad = "select * from Proveedor where upper(RUC) like  upper('" + txtbusqueda.Text + "%')";
            }
            else if (cbobuscar.Text == "ID")
            {
                cad = "select * from Proveedor where upper(prov_id) like upper('" + txtbusqueda.Text + "%')";
            }
            else
                cad = "select * from Proveedor where upper(productos) like upper('" + txtbusqueda.Text + "%')";
            grilla(cad);
        }
    }
}
