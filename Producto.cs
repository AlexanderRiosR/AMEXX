...
namespace Verduleria
{
    public partial class Producto : Form
    {
        ...
        private void Producto_Load(object sender, EventArgs e)
        {
            ...
            string cad = "select * from producto ";
            grilla(cad);
        ...
        { 
            SqlCommand sqlprov; 
            SqlDataAdapter dataprov; 
            DataSet dsprov; 
            SqlConnection con; 
            con = new SqlConnection();
            //@"Data Source=localhost\SQL2022; Initial Catalog=verduleria; User ID = sa; Password = lab02; TrustServerCertificate = True";
            con.ConnectionString = @"Data Source=Melita\SQLEXPRESS; Initial Catalog=verduleria; User ID = sa; Password = *Emelin*!24; TrustServerCertificate = True"; 
            con.Open(); 
            dataprov = new SqlDataAdapter(); 
            dsprov = new DataSet(); 
            sqlprov = new SqlCommand(cadena, con); 
            dataprov.SelectCommand = sqlprov; 
            dataprov.TableMappings.Add("Table", "producto"); 
            dataprov.Fill(dsprov); 
            dgvproducto.DataSource = dsprov; 
        ...
        private void txtbusqueda_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                cad = "select * from producto where upper(prod_descripcion) like  upper('" + txtbusqueda.Text + "%')"; 
             }
            else
            {
                cad = "select * from producto where upper(Codigo) like upper('" + txtbusqueda.Text + "%')";
            }
            grilla(cad);
        }
    }
